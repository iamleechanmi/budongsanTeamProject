...
is
begin
    insert into tblNotice (seq, subject, detail, imageURL, regDate, adminSeq, delFlag, readcount) 
    values (seqNotice.nextVal, psubject, pdetail, pimageURL, default, padminSeq, 0, default);
    commit;
end procAddNotice;
...
-- 공지사항 총 페이지수 계산하는 함수
-------------------------------------------------------------------------------
create or replace function fnNoticeMaxPage(
...
begin
    -- 최대 페이지수를 변수에 저장한다.
    select ceil((select count(*) from vwNotice) / 10) into vmaxPage from dual;
...
-- 1-2. 공지사항 view (v) 이거 하나로만 가도 될 것 같습니다
create or replace view vwNotice
as
select
    ntc.seq as seq, /* 공지사항 번호 */
    adm.seq as adminseq, /* 작성한 관리자 번호 */
    rtrim(ntc.subject) as subject, -- 공지사항 제목. notice -> ntc
    ntc.detail as detail,
    ntc.imageURL as imageurl,
    ntc.regDate as regdate,
    adm.name as adminname,
    ntc.readcount as readcount, -- 조회수
    (sysdate - ntc.regdate) * 24 as gap -- 시간 차
from tblNotice ntc
    inner join tblAdmin adm
        on adm.seq = ntc.adminSeq
            where delFlag <> 1;
-------------------------------------------------------------------------------
-- 공지사항 글 목록 조회(최대 10개 목록 출력된다. 최신글 순, 페이지수 필요)
-------------------------------------------------------------------------------
create or replace procedure procListNotice(
    ppage number, -- 페이지수
...
begin
    open pcursor for select
                            ...
                            readcount, -- 조회수
                            gap -- 시간 차
                        from (select rownum as rnum, n.* from vwnotice n order by regdate desc)
                            where rnum between 10*(fnNoticeMaxPage(ppage)-1)+1 and fnNoticeMaxPage(ppage)*10;
end procListNotice;
-------------------------------------------------------------------------------
--2-1. 공지사항 목록보기
